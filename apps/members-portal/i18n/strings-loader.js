...
class StringsLoader {
  ...
  async load(locale = 'is') {
    ...
    const xmlPath = `/i18n/values-${locale}/portal-strings.xml?v=20251215`;
...
export const R = {
  ...
  translatePage: () => {
    ...
    document.querySelectorAll('[data-i18n]').forEach(element => {
      ...
      const value = stringsLoader.get(key);
      if (value && value !== key) {
    ...
    debug.log(`Translated page with ${Object.keys(stringsLoader.strings).length} strings`);
  }
};
