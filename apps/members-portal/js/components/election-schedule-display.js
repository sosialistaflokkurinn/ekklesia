 ...
 *
 * Usage:
 *   import { createScheduleDisplay } from './components/election-schedule-display.js';
...
import { formatDateIcelandic } from '../utils/util-format.js';
 ...
 * Can be overridden by passing strings option
 */
const DEFAULT_STRINGS = {
  start_label: 'Kosning hefst',
  end_label: 'Kosning lýkur',
  status_aria_label: 'Staða',
  status_upcoming: 'Hefur ekki byrjað',
  status_active: 'Kosning í gangi',
  status_closed: 'Kosningu lokið',
  status_unknown: 'Óþekkt staða'
};
 ...
 * @param {string} options.startLabel - Label for start time (deprecated, use strings)
 * @param {string} options.endLabel - Label for end time (deprecated, use strings)
...
export function createScheduleDisplay(options = {}) {
  ...
  const startItem = el('div', 'election-detail__schedule-item', {},
    el('span', 'election-detail__schedule-label', {}, strings.start_label),
    startValue
  );

  const endValue = el('span', 'election-detail__schedule-value');
  const endItem = el('div', 'election-detail__schedule-item', {},
    el('span', 'election-detail__schedule-label', {}, strings.end_label),
    endValue
  );
  ...
  function updateStatus(status) {
    ...
    const statusLabels = {
      upcoming: strings.status_upcoming,
      active: strings.status_active,
      closed: strings.status_closed
    };

    statusText.textContent = statusLabels[status] || strings.status_unknown;
