# Ekklesia Data Quality & UX Guide

**Last Updated**: 2025-11-04
**Status**: ✅ Active - Required for Frontend Development
**Purpose**: Principles and patterns for balancing data quality with user experience
**Location**: Relocated from `docs/development/DATA_QUALITY_POLICY.md`
...
### Pattern 5: Status Feedback (Ekklesia Standard)

**Use case**: Show user that async operations are working
...
**Why**: Users need to know when async operations (save, load) are happening

**User sees**: Loading spinner → Success checkmark → Auto-clear (not cluttered)

**Data quality**: No impact on data, but improves user confidence in data being saved
...
Data quality patterns integrate with HTML standards:

```html
<!-- Form with validation feedback -->
<div class="form__group">
  <label for="input-phone" class="form__label" id="label-phone">Loading...</label>
  <input
    type="tel"
    id="input-phone"
    class="form__input"
  ...
  />
  <span class="profile-field__status" id="status-phone"></span>
  <span id="error-phone" class="form__error" role="alert"></span>
</div>
```

**Key Elements**:
- `type="tel"` - Semantic input type (mobile shows number pad)
- `aria-describedby` - Links error message for screen readers
- `role="alert"` - Announces errors to assistive technology
- Status icon for async feedback (our standard pattern)

---

### JavaScript Standard Compliance

Data quality code follows JavaScript standards:

```javascript
/**
 * Validate and save phone number with auto-correction
 *
 * @param {HTMLInputElement} input - Phone input element
 * @param {HTMLElement} statusIcon - Status feedback icon
 * @returns {Promise<boolean>} True if saved successfully
 */
async function validateAndSavePhone(input, statusIcon) {
  let value = input.value.trim();

  // Auto-prepend '+' for E.164 format
  if (value.length > 0 && /^\d/.test(value)) {
    value = '+' + value;
    input.value = value;  // Show user the correction
  }

  // Validate
  if (!value.startsWith('+') || value.length < 6) {
    const errorMsg = R.string.error_phone_invalid;
    document.getElementById('error-phone').textContent = errorMsg;
    return false;
  }
  ...
  try {
    ...
    const errorMsg = R.format(R.string.error_save, error.message);
    document.getElementById('error-phone').textContent = errorMsg;
    return false;
  }
}
...
**BEM Standard**:
- Block: `.profile-field`, `.form`
- Elements: `.profile-field__status`, `.form__error`
- Modifiers: `.profile-field__status--loading`, `.profile-field__status--success`
...
- **Icelandic Postal Codes**: https://www.postur.is/en/individuals/zip-codes/

**Internal References**:
- **Django Backend Requirements**: `docs/requirements/DJANGO_FOREIGN_ADDRESS_REQUIREMENTS.md`
...
| 1.1 | 2025-11-04 | Relocated to standards/ and integrated with code standards |
